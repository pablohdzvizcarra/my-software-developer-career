...
Today I started the Bitwise operations course section. I am very interesting on this section because on my work we do a lot of bit manipulation operations with Java in our code to talk with the low-level components, I know that the course that I am doing is with C, but the concepts are abstract.
...
Today I worked into fix a code issue that is related with poor performance in a logger feature. The problem with this code was does not implement truncation logic for the logs files, the code just removed N number of bytes at the start of the file and next append the new entries. This code behavior generate performance issues and because we use a lock mechanism every time we want to append a new entry to the log, you ca imagine the bottleneck in a high throughput environment. I fixed the truncation logic and with this change the code works as expected.
