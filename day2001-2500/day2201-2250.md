...
Today I worked into fix a code issue that is related with poor performance in a logger feature. The problem with this code was does not implement truncation logic for the logs files, the coe just removed N number of bytes at the start of the file and next append the new entries. This coe behavior generate performance issues and because we use a lock mechanism every time we want to append a new entry to the log, you ca imagine the bottleneck in a high throughput environment. I fixed the truncation logic and with this change the code works as expected.
