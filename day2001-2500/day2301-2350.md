# Days 2301-2350: Systems Programming, OS Kernels & Networking Fundamentals

During this 50-day period, learning focused on low-level systems programming in C/C++, Linux kernel architecture (namespaces, context switching, system calls, and process/thread management), networking fundamentals (TCP/IP, sockets, kernel queues, and asynchronous I/O with `select`/`epoll`/`kqueue`/`io_uring`), and storage systems (SSD FTL internals, file system block sizes, and IBM DS8000 storage). Key project milestones included building a C++ Event Engine, creating a CLI SSD simulator, completing pointer arithmetic modules from *C Programming: A Modern Approach*, and leveraging terminal workflows with Tmux.
